local function get_jdtls()
    ...
    -- TODO: wget https://projectlombok.org/downloads/lombok.jar -O /home/<x>/.local/share/nvim/mason/packages/jdtls/lombok.jar 
    -- Download manolaly if not exist
...
local function setup_jdtls()
    ...
    -- Only append the lombok agent if the jar actually exists on disk
    if vim.uv.fs_stat(lombok) then
        table.insert(cmd, '-javaagent:' .. lombok)
    end

    -- Append the remaining required args
    vim.list_extend(cmd, {
        '-jar',
        launcher,
        '-configuration',
